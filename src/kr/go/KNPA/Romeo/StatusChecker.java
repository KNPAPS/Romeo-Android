...
public class StatusChecker {
	...
	public int getUserStatus() {
		...
		String userHash = UserInfo.getPref(mContext, UserInfo.PREF_KEY_USER_HASH );
		if ( userHash == null ) {
		...
		Data data = new Data();
		data.add(hm);
		Payload requestPayload = new Payload(EventEnum.USER_GET_USER_INFO);
		requestPayload.setData(data);
		Connection conn = new Connection.Builder( requestPayload.toJson() ).build();
		conn.request();
		Payload responsePayload = new Payload(conn.getResponsePayload());
		/** 데이터 가져오기 끝 */
		 ...
		 * 서버에 등록조차 안되어 있는 상태임
		 */
		if ( responsePayload.getStatusCode() == StatusCodeEnum.SUCCESS ) {
	...
	public int getDeviceStatus() { 
		//기기에 유저의 기존 정보가 등록되어 있지 않으면 바로 unregistered 창으로 돌린다.
		String userHash = UserInfo.getPref(mContext, UserInfo.PREF_KEY_USER_HASH );
		String uuid = UserInfo.getPref(mContext, UserInfo.PREF_KEY_UUID );
		String regid = UserInfo.getPref(mContext, UserInfo.PREF_KEY_REG_ID );
		...
		Data data = new Data();
		data.add(hm);
		Payload requestPayload = new Payload(EventEnum.DEVICE_IS_REGISTERED);
		requestPayload.setData(data);
		Connection conn = new Connection.Builder( requestPayload.toJson() ).build();
		conn.request();
		Payload responsePayload = new Payload(conn.getResponsePayload());
