...
public class MemberListSearchAdapter extends CellNodeTreeAdapter implements OnItemClickListener {
	...
	{
		return objectForRowAtIndexPath(getIndexPathFromPosition(position));
	}
	...
	public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		if (convertView == null || getItemViewType(position) != node.type())
		{ // re-usable test
			switch (node.type())
			{
		...
		if (node.type() == CellNode.CN_DEPARTMENT)
		{
			...
			case CellNode.NCHECK:
				control.setBackgroundResource(R.drawable.circle);
				break;
	...
	protected void getSubNodes(final CellNode nodeClicked)
	{
		...
		new Thread(new Runnable() {
			...
			public void run()
			{
				...
				{
				case CellNode.HCHECK:
					status = CellNode.NCHECK;
					break; // NEVER REACH

				case CellNode.FCHECK:
					status = CellNode.FCHECK;
					break;
				default:
				case CellNode.NCHECK:
					status = CellNode.NCHECK;
				...
				for (int i = 0; i < deps.size(); i++)
				{
					CellNode node = new CellNode().status(status).type(CellNode.CN_DEPARTMENT).idx(deps.get(i).idx).isRoot(false).isUnfolded(false).index(nodeClicked.children().size())
							.parent(nodeClicked);
				...
				for (int i = 0; i < users.size(); i++)
				{
					...
					CellNode node = new CellNode().status(status).type(CellNode.CN_USER).idx(users.get(i).idx).isRoot(false).isUnfolded(false).index(nodeClicked.children().size()).parent(nodeClicked);
					nodeClicked.append(node);
				}
