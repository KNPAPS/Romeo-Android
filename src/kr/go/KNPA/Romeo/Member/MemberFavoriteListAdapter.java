...
public class MemberFavoriteListAdapter extends CursorAdapter implements OnItemClickListener {
	...
	public void bindView(View v, final Context ctx, Cursor c) {
		...
		idx = c.getString(c.getColumnIndex(MemberProcManager.COLUMN_FAVORITE_IDX));
		// 즐겨찾기 이름
		title = c.getString(c.getColumnIndex(MemberProcManager.COLUMN_FAVORITE_NAME));
		// 그룹인지 아닌지
		isGroup = c.getInt(c.getColumnIndex(MemberProcManager.COLUMN_FAVORITE_IS_GROUP)) == 1 ? true : false;
			...
			searchButton.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(_collect.contains(idx)) {
						...
						// 뺀다. 체크 해지.
						_collect.remove(idx);
						v.setBackgroundResource(R.drawable.circle_arrow_right_gray);
					} else {
