...
public class CellNode {
	...
	private boolean detAllSiblingsAre(int status)
	{
		...
		for (int i = 0; i < sibs.size(); i++)
		{
			...
			if (sib.indexPath().equals(this.indexPath()))
				continue;
	...
	/*
	 * private void detParentStatusChecked() { boolean flag = false;
	 * ArrayList<CellNode> sibs = this.parent().children();
	 * 
	 * for(int i=0; i<sibs.size(); i++) { CellNode sib = sibs.get(i);
	 * if(sib.indexPath().equals(this.indexPath())) continue; if(sib.status() !=
	 * NCHECK) flag = true; }
	 * 
	 * if(flag == true) { if(this.isRoot() == true) return; Condition c = new
	 * Condition().isRoot(false); ArrayList<CellNode> _ps = this.parents(c);
	 * for(int i=0; i<_ps.size(); i++) { _ps.get(i).status(HCHECK); }
	 * this.status(NCHECK); } else { if(this.isRoot() == true) return;
	 * this.status(NCHECK); this.parent().detParentStatusChecked(); } }
	 * 
	 * private void detParentStatusUnchecked() { boolean flag = false;
	 * ArrayList<CellNode> sibs = this.parent().children();
	 * 
	 * for(int i=0; i<sibs.size(); i++) { CellNode sib = sibs.get(i);
	 * if(sib.indexPath().equals(this.indexPath())) continue; if(sib.status() !=
	 ...
	 * Condition().isRoot(false); ArrayList<CellNode> _ps = this.parents(c);
	 * for(int i=0; i<_ps.size(); i++) { _ps.get(i).status(HCHECK); }
	 * this.status(FCHECK); } else { if(this.isRoot() == true) return;
	 * this.parent().detParentStatusUnchecked(); this.status(FCHECK); } }
	 */
	public static ArrayList<String> collect(CellNode rootNode)
	{
		 ...
		 * if(_selectedDeps.get(i).children() != null &&
		 * _selectedDeps.get(i).children().size() >0)
		 * selectedDeps.add(_selectedDeps.get(i)); }
		 * 
		 * 
		 * for( int i=0; i<selectedDeps.size(); i++) { ArrayList<User> _us =
		 * MemberManager
		 * .sharedManager().getDeptMembers(selectedDeps.get(i).idx(), true);
		 * for(int j=0; j<_us.size(); j++) { _result.add(_us.get(j).idx); } }
		 * 
		 * ArrayList<String> result = new ArrayList<String>(); Iterator<String>
		 * itr = _result.iterator(); while(itr.hasNext()) {
		 * result.add(itr.next()); }
		 */
		 ...
		 * 
		 * for(int i=0; i<selectedNodes.size(); i++) { CellNode node =
		 * selectedNodes.get(i); if(node.type() == CellNode.CN_DEPARTMENT) {
		 * ArrayList<User> users =
		 * MemberManager.sharedManager().getDeptMembers(node.idx(), true);
		 * for(int ui=0; ui<users.size(); i++) { boolean isExists = false; User
		 * user = users.get(ui);
		 * 
		 * for(int j=0; j< result.size(); j++) { if(
		 * result.get(j).equalsIgnoreCase(user.idx) ) { isExists = true; break;
		 * // ui index를 가진 다음 for 문으로 간다. } }
		 * 
		 * if(isExists != true) result.add(user.idx); } } else { boolean
		 * isExists = false;
		 * 
		 * for(int j=0; j< result.size(); j++) { if(
		 * result.get(j).equalsIgnoreCase(node.idx()) ) { isExists = true;
		 * break; // ui index를 가진 다음 for 문으로 간다. } }
		 * 
		 * if(isExists != true) result.add(node.idx());
		 * 
		 * } }
		 */

		final HashSet<String> _result = new HashSet<String>();
		...
		{
			final CellNode node = selectedNodes.get(ni);
